...
class GtkController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id_gtk', 	'nama', 	'nik', 	'nip', 	'jk', 	'tempat_lahir', 	'tgl_lahir', 	'alamat', 	'status');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text=$this->search;
			$db->orWhere('id_gtk',"%$text%",'LIKE');
		...
		$records = $db->get('gtk', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'gtk', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'gtk' , false );
				}
				if($db->getLastError()){
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('id_gtk' , $rec_id);
		}
		$record = $db->getOne( 'gtk', $fields );
		if(!empty($record)){
			render_json($record);
		}
		else{
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('gtk',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
