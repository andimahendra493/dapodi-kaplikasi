    <template id="profil_penggunaList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Profil Pengguna</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/profil_pengguna/add'">
                            <i class="fa fa-plus"></i>
                            Add New Profil Pengguna
                            </router-link>
                        </div>
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/profil_pengguna">Profil Pengguna</router-link>
                                            </li>
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.id_profil" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    <th v-if="listsequence" class="td-sno">{{index + 1}}</th>
                                                    <td><router-link :to="'/profil_pengguna/view/' +  data.id_profil">{{data.id_profil}}</router-link></td>
                                                    <td> {{ data.nama }} </td>
                                                    <td> {{ data.nip }} </td>
                                                    <td> {{ data.jk }} </td>
                                                    <td> {{ data.alamat }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/profil_pengguna/view/' + data.id_profil">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/profil_pengguna/edit/' + data.id_profil">
                                                            <i class="fa fa-edit"></i> 
                                                            </router-link>
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.id_profil,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.id_profil"><i class="fa fa-times"></i></span>
                                                                <clip-loader :loading="deleting == data.id_profil" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="profil_pengguna/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var Profil_PenggunaListComponent = Vue.component('profil_penggunaList', {
		template: '#profil_penggunaList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'profil_penggunalist',
			},
			apipath : {
				type : String,
				default : 'profil_pengguna/list',
			},
			tablestyle: {
				type: String,
			...
			allSelected: function(){
				...
				if(this.allSelected == true){
					for (var i in this.records){
						var id = this.records[i].id_profil;
